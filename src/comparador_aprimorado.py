"""
Módulo de comparação aprimorado entre documentos com detecção inteligente de alterações.
...
class ComparadorAprimorado:
    """Classe responsável pela comparação aprimorada entre documentos."""
    ...
    def comparar_documentos(self, doc1: Documento, doc2: Documento) -> ResultadoComparacao:
        ...
        logger.info(f"Iniciando comparação aprimorada entre {doc1.nome_arquivo} e {doc2.nome_arquivo}")
        ...
        for seg1, seg2 in segmentos_alinhados:
            comparacao = self._comparar_segmentos_aprimorado(seg1, seg2)
            if comparacao:
        ...
        logger.info(f"Comparação aprimorada concluída: {len(comparacoes)} comparações válidas")
        return resultado
    
    def _pre_processar_segmentos(self, segmentos: List[Segmento]) -> List[Segmento]:
        """Pré-processa segmentos para melhor alinhamento (normalização + segmentação extra)."""
        processados = []
        # Agrupar blocos curtos e normalizar texto
    ...
    def _agrupar_blocos_curtos(self, segmentos: List[Segmento]) -> List[Segmento]:
        """Quebra segmentos em frases individuais com contexto."""
        if not segmentos:
            return segmentos
        ...
        for segmento in segmentos:
            ...
            # Se é um título ou cláusula, manter como está
            tipo_atual = self._classificar_tipo_conteudo(texto)
            if tipo_atual in ["titulo_principal", "subtitulo", "clausula"]:
                frases_processadas.append(segmento)
                continue
            
            # Quebrar em frases individuais
            ...
            for i, frase in enumerate(frases):
                ...
                # Criar contexto para a frase
                contexto = self._criar_contexto_frase(frases, i, segmento)
                
                # Criar segmento de frase
                segmento_frase = Segmento(
                    texto=frase.strip(),
                    pagina=segmento.pagina,
                    posicao=segmento.posicao + i,  # Posição relativa
                    tipo="frase",
                    contexto=contexto
                )
                
                frases_processadas.append(segmento_frase)
        
        return frases_processadas
    
    def _quebrar_em_frases(self, texto: str) -> List[str]:
        """Quebra texto em frases individuais."""
        # Padrões para quebrar frases
        padroes_quebra = [
            r'[.!?]\s+',  # Pontuação final seguida de espaço
            r'[.!?]\n+',  # Pontuação final seguida de quebra de linha
            r';\s+',      # Ponto e vírgula seguido de espaço
            r':\s+',      # Dois pontos seguido de espaço
            r'\n+',       # Quebras de linha
        ]
        
        # Combinar todos os padrões
        padrao_completo = '|'.join(padroes_quebra)
        
        # Quebrar o texto
        frases = re.split(padrao_completo, texto)
        
        # Limpar frases vazias e normalizar
        frases_limpas = []
        for frase in frases:
            frase_limpa = frase.strip()
            if frase_limpa and len(frase_limpa) > 3:  # Frases muito curtas são ignoradas
    ...
    def _normalizar_texto(self, texto: str) -> str:
        """Normaliza texto removendo diferenças irrelevantes."""
    ...
    def _comparar_segmentos_aprimorado(self, seg1: Optional[Segmento], 
                                      seg2: Optional[Segmento]) -> Optional[ComparacaoSegmento]:
        """Compara dois segmentos com detecção aprimorada."""
